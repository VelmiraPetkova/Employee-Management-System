...
class TestUserManager(TestBase):
    ...
    @patch.object(SEService, 'verify_email_identity')
    @patch.object(SEService, 'create_client')
    def test_create_user_success(self,mock_create_client,mock_verify_email_identity):
        ...
        mock_verify_email_identity.assert_called_once_with(test_mail)
        user = UserModel.query.all()
        assert len(user) == 1
